...
{
    FILE *fp;
    int N=0, grades=0, sum = 0, median = 0, fixed_median =0;
   ...
   //checks if there is one argument given as needed and that it's not empty
    ...
    median--;
    int temp = median;
    //calculating the median if the no. of students is even
    while(median<=MAX_GRADE)
    {
        median++;
        if(hist[median] != 0)
       {
           fixed_median =  ((temp + median)/2);
           break;
       }
    }
    //closing the file, printing the median to the stdout file
    fclose(fp);
    fprintf(stdout,"%d\t",fixed_median);
return 0;
}
